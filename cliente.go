...
func mainCliente() {
	// Verifica argumentos
	if len(os.Args) < 2 {
		fmt.Println("Uso: go run cliente.go <nome_do_jogador> [endereco_servidor]")
		fmt.Println("Exemplo: go run cliente.go Joao localhost:1234")
		os.Exit(1)
	}
...
func (c *ClienteJogo) buscarAtualizacoes() {
	for {
		time.Sleep(100 * time.Millisecond) // Busca atualizações a cada 100ms
...
func (c *ClienteJogo) desenharBarraDeStatus() {
	...
	status := fmt.Sprintf("Jogador: %s | Posição: (%d, %d) | Seq: %d", 
	...
	// Instruções
	instrucoes := "Use WASD para mover, E para interagir, ESC para sair"
